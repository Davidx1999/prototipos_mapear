...
const Login = ({ 
  ...
  return (
    <div className="min-h-screen w-full flex p-[16px] md:p-[24px]" style={{ backgroundColor: colors.neutral[0], fontFamily: 'Montserrat, sans-serif' }}>
      <div className="w-full relative flex items-center justify-center rounded-[16px] md:rounded-[24px] overflow-hidden transition-all duration-500" style={{ backgroundColor: colors.primary.base, backgroundImage: isHighContrast ? 'none' : 'url("https://images.unsplash.com/photo-1619252584172-a83a949b6efd?q=80&w=1074&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundBlendMode: 'multiply' }}>
        <div className="relative z-10 p-[32px] md:p-[48px] rounded-[16px] w-full max-w-[420px] shadow-2xl flex flex-col items-center transition-colors duration-500 m-[16px]" style={{ backgroundColor: colors.neutral[0] }}>
          ...
          <LogoFgv stacked={true} isHighContrast={isHighContrast} colors={colors} />
          <div className="mt-[32px] text-center w-full">
            <h1 className="text-[20px] md:text-[24px] font-bold" style={{ color: colors.neutral[7] }}>Olá, boas-vindas!</h1>
            <p className="text-[13px] md:text-[14px] mt-[8px]" style={{ color: colors.neutral[5] }}>Por favor, realize seu login ou solicite cadastro</p>
          </div>
          <form className="w-full mt-[32px] flex flex-col gap-[16px]" onSubmit={handleLogin}>
            ...
            <Button variant="tertiary" type="button" className="w-full" onClick={() => { setIsLoggedIn(true); setCurrentScreen('dashboard'); }}>Solicitar Cadastro</Button>
          </form>
          <p className="text-[10px] mt-[48px]" style={{ color: colors.neutral[4] }}>Todos os direitos reservados © 2026 FGV DGPE</p>
        </div>
      </div>
    </div>
  );
